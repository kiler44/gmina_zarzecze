...
class MessageSelector
{
    ...
    public function choose($message, $number, $locale)
    {
        ...
        foreach ($parts as $part) {
            ...
            }  elseif (preg_match('/^(?P<interval>'.  Variable::getIntervalRegexp().')\s*(?P<message>.*?)$/x', $part, $matches)) {
                $variableRules[$matches['interval']] = $matches['message'];
        ...
        // try to match an variable rule, then fallback to the standard ones
        foreach ($variableRules as $variable => $m) {
            if (Variable::test($number, $variable)) {
                return $m;
            }
        }
