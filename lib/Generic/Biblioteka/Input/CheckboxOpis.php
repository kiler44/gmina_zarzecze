...
class CheckboxOpis extends Input
{
	...
	protected $tpl = '
	<input type="checkbox" class="{{$klasa}} form-check-input" name="{{$nazwa}}" value="1" id="checkbox_{{$nazwa}}" {{IF $zaznaczony}}checked="checked"{{END}} {{$atrybuty}}/>
	<label for="checkbox_{{$nazwa}}" class="form-check-label">{{$opis}}</label>
	<span id="{{$nazwa}}"></span>
