...
class MultiTextSelect extends Input
{
	protected $katalogSzablonu = 'MultiTextSelectNew';
	protected $tpl = '
	...
	function pobierzHtml()
	{
		...
		$dane = array(
			...
			'etykieta_podzial' => $this->tlumaczenia['input_multi_text_select_etykieta_podzial'],
			'etykieta_dodaj_pole' => $this->tlumaczenia['input_multi_text_select_etykieta_dodaj_pole'],
		...
		foreach ($wartosc as $klucz => $wartosc)
		{
			...
			foreach($lista as $opcja => $etykieta)
			{
				$dane['wartosci'][$licznik]['lista'][] = array(
					'opcja' => $opcja,
					'etykieta' => $etykieta,
					'selected' => ($opcja == $wartosc) ? true : false,
				);
			}
			$licznik++;
