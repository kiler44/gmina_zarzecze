 ...
 *
 * - url_upload - ajaxowy adres url na ktory ma zostac wyslane zdjecie, przyklad: Router::urlAjax('Http', $this->kategoria, 'zapiszZdjecie'),
 *
 * - url_plikow - sciezka do pliku minaturki, przyklad: '/_public/ogloszenia/'
 *
 ...
 * @package biblioteki
 */
class MultiUploadZdjec extends Input
{
	protected $katalogSzablonu = 'MultiUploadPhotosNew';
...
<li id="{{$id}}"> <img src="{{$url}}" alt="{{$opis_zdjecia}}" width="92" height="88" />
  <div class="edition-area">
	...
	{{IF $ikonaPopraw}}<a href="javascript:void(0)" class="przyciskEdytuj edit_thumb" rel="{{$linkPopraw}}"><span ></span></a>{{END}} </p>
...
<li id="{id}"> <img src="" alt="" width="92" height="88" />
  <div class="edition-area">
	<p><input type="checkbox" name="{{$usun_nazwa}}" value="{{$usun_wartosc}}" />
	  <a href="javascript:void(0)" class="remove {{$funkcja_usun}}" rel="{{$id}}">{{$etykieta_usun}}<span ></span></a> </p>
...
{{BEGIN funkcja}}
<script type="text/javascript" src="/_szablon/biblioteki/fileuploader.js"></script>
		...
		$(this).click(function () {
			...
			$.fn.colorbox({
				...
				onComplete: function () {
					...
					{
						$(\'#cboxLoadedContent\').html(\'{{$input_multi_upload_etykieta_blad}}\');
...
';
	...
	function pobierzWartosc()
	{
		...
		foreach($dane as $klucz => $wartosc)
		{
            ...
            if (strpos($klucz, $this->nazwa . '-zdjecie_id') !== false)
            {
                $k = explode('-', $klucz);
                $this->wartosc['pliki'][$k[0]]['zdjecie_id'] = $this->filtrujWartosc($wartosc);
            }
		...
		if (isset($dane[$this->nazwa . '_kolejnosc']))
		{
			/**
	...
	protected function parsujSzablon($szablon, $plik = null, $url = null)
	{
		if ($plik != null && $url != null)
		{
            ...
            $dozwoloneMiniatury[$url.'/'.$plik['kod']] = $this->parametry['rozmiary_miniaturek'];

            Cms::inst()->sesja->dozwoloneMiniatury = $dozwoloneMiniatury;

			$linkPoprawMiniature = isset($plik['linkPoprawMiniature']) ? $plik['linkPoprawMiniature'] : '';

			$dane = array(
				'url' => $url . '/' . $prefix . $plik['kod'],
				'opis_zdjecia' => $plik['kod'],
				'nazwa' => $plik['nazwa'],
				'rozmiar' => formatbytes($plik['rozmiar'], 'KB'),
				'opis_nazwa' => $plik['id'] . '-' . $this->nazwa . '-opis',
				'id_nazwa' => $plik['id'] . '-' . $this->nazwa . '-zdjecie_id',
				'id_wartosc' => $plik['zdjecie_id'],
				'opis_wartosc' => $opis,
				'usun_nazwa' => $plik['id'] . '-' . $this->nazwa . '-usun',
				'usun_wartosc' => $plik['id'],
				'funkcja_usun' => $this->nazwa . '-usun',
				'linkPopraw' => $linkPoprawMiniature,
				'ikonaPopraw' => $linkPoprawMiniature == '' ? false : true,
				'id' => $plik['id'],
				'etykieta_usun' => $this->tlumaczenia['input_multi_upload_zdjec_etykieta_usun'],
				'etykieta_popraw' => $this->tlumaczenia['input_multi_upload_zdjec_etykieta_croper'],
			);
		}

		$dane['etykieta_opis'] = $this->tlumaczenia['input_multi_upload_zdjec_etykieta_opis'];
	...
	public function pobierzHtml()
	{
		...
		{
			if (isset($this->parametry['lista'][0]['kolejnosc']))
			{
		...
		$this->szablon->ustawGlobalne(array(
			...
			'wyswietlaj_drop_area' => $this->parametry['wyswietlaj_drop_area'],
			'etykieta_usun' => $this->tlumaczenia['input_multi_upload_zdjec_etykieta_usun'],
			'szablon' => trim(preg_replace('/\s+/', ' ', $this->parsujSzablon('szablon_zdjecie_2', array(
				'etykieta_usun' => $this->tlumaczenia['input_multi_upload_zdjec_etykieta_usun'],
			)))),
		));
