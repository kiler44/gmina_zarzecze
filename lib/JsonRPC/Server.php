...
class Server
{
	...
	public function setApiInfoRequest()
	{
		$client = new Client('http://bkt.pl/api');
		$wynik = $client->execute('getApiInfo');
		dump(json_decode($wynik));
		die;
	}
	...
	public function execute($password = '')
	{
		try {
			...
			if ($password != '')
			{
				if (isset($this->payload['params']['__token']))
				{
					$token = $this->payload['params']['__token'];
					unset($this->payload['params']['__token']);
				}
				else
				{
					$this->sendForbiddenResponse();
				...
				$token2 = hash('haval256,3', $checksum.hash('haval128,3', $password));
				if ($token != $token2)
				{
					$this->sendAuthenticationFailureResponse();
					exit;
			...
			{
				if (isset($this->payload['params']['time'])){
					unset($this->payload['params']['time']);
				}
				if (isset($this->payload['params']['__token'])){
					unset($this->payload['params']['__token']);
				}
