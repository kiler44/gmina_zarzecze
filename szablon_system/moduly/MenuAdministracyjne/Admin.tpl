...
$(document).ready(function(){
	...
	$('.rozwin').on('click', function (){
		$('li[attr-parent="'+$(this).attr('attr-id')+'"]').toggle(500);
	});
...
<div class="sidebarHeader"><h5><a href="{{$link_glowna}}" title="{{$etykieta_strona_glowna}}" class="link"><i class="icon icon-home"></i> <span>{{$etykieta_menu}}</span></a></h5></div>
	...
	{{END}}
		...
		{{BEGIN element}}
			<li class="{{IF $aktywny}}active{{END}} {{$klasa_wiersza}}" attr-parent="{{$rodzic_id}}" attr-id="{{$kategoria_id}}" {{IF $ukryj}} style="display: none;" {{END IF}}>
				<a href="{{$url}}" title="{{$nazwaPelna}}" class="{{$klasa}}">{{IF $ukryj}}-{{END IF}} {{IF $ikona}}<i class="icon {{$ikona}}"></i>{{END}} <span>{{$nazwaPelna}}</span> </a>
			</li>
		{{END}}
		{{BEGIN elementRozwijalny}}
			<li class="{{IF $aktywny}}active{{END}} {{$klasa_wiersza}} rozwin cursorPointer" attr-id="{{$kategoria_id}}">
				<a  title="{{$nazwaPelna}}" class="{{$klasa}}">{{IF $ikona}}<i class="icon {{$ikona}}"></i>{{END}} <span>{{$nazwaPelna}}</span> &#8595; </a>
			</li>
		{{END}}
