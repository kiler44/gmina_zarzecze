...
<!--
	...
	$("#zaznacz_wszystkie, #zaznacz_btn").click(function()
	{
		...
		else
		{
			...
			$(".grid").checkCheckboxes();
			$(".grid tr.wiersz").addClass("clicked");
		}
	});

	//Obsługa przycisków grupowych
	$(".grid .akcja_grupowa:not(#zaznacz_wszystkie, #zaznacz_btn, #odwroc_zaznaczenie) ").click(function(){
		...
		{
			$(".grid").append('<form id="grid_actions_form" method="post" action="' + $(this).attr("href") + '"></form>');
	...
	$(".akcje_grupowe").click(function(){
		if($(".grid input[type='checkbox']:checked").length == 0) return false;
	...
	$(".grid #odwroc_zaznaczenie").click(function() {
		$(".grid").toggleCheckboxes();
		zaznaczone = true;
...
{{BEGIN wiersz}}
	...
	{{BEGIN kolumna_przyciski}}
		<td class="{{$klasa}} przyciski" valign="middle">
			<div class="btn-group">
			{{BEGIN przycisk}}<a class="btn tip-top" href="{{$url}}" title="{{$etykieta}}" {{$parametry}}>{{BEGIN ikona}}<i class="{{$ikona}}"></i>{{END}}</a> {{END}}
		...
		<td colspan="{{$ilosc_kolumn}}">
			<div class="pull-left">
				{{BEGIN przycisk}}<a href="{{$url}}" class="akcja_grupowa" title="{{$etykieta}}" {{$parametry}}>
					{{BEGIN ikona}}<img src="/_system/admin/ikony/{{$ikona}}" alt="{{$etykieta}}"/>{{END}}<!-- {{ if($etykieta,'<span>') }}{{$etykieta}}{{ if($etykieta,'</span>') }} -->
				</a>
				{{END}}
			</div>
