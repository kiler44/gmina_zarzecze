CKEDITOR.plugins.add( 'charcount',
{
   init : function( editor )
   {
      ...
      var defaultFormat = '<span class="cke_charcount_count"  style="display:block; float:right; margin-top:6px; margin-right:5px; color:#60676A; font-style: italic;">%count%</span>';
      ...
      {   
         ...
         {
            var count = editor.getData().replace(/(<([^>]+)>)/ig,"").replace(/(&nbsp;)/ig, 'd').replace(/[\n\r\t]/g,'d').replace(/<(?:.|\s)*?>/g, '').length;
