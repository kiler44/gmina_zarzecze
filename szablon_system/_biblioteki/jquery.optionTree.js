...
$.fn.optionTree = function(tree, options) {
    ...
    var isPreselectedFor = function(clean, v) {
      ...
      if ($.isArray(options.preselect[clean])) {
        ...
        // zmiany
		key = $.inArray(v, options.preselect[clean]);
		if (key != -1) {     	
			options.preselect[clean].splice(key, 1);
		    return true;
		 } else {
		    return false;
		 }
      ...
      // zmiany
     if (options.preselect[clean] == v) {     	
		options.preselect.splice(clean, 1);
        return true;
     } else {
        return false;
     }
    ...
    return this.each(function() {
        ...
        if (typeof tree == "object") { // many options exists for current nesting level
            ...
            if (options.rozmiar)
                $select.attr('size',options.rozmiar);
