...
{{BEGIN produkt_dodany}}
	<li class="item {{IF $_first}}bold{{END}}">
		<span class="prd" data-unit="{{$wartosc_jednostka}}">{{$wartosc_nazwa}}</span>{{IF $wartosc_multiple == "true"}}<span class="qty"><input type="text" class="spn" data-unit="{{$wartosc_jednostka}}" name="{{$nazwa}}_qty[]" value="{{$wartosc_ilosc}}"  /></span>{{ELSE}}<input type="hidden" name="{{$nazwa}}_qty[]" value="1"/>{{END}}{{IF $wartosc_usun == true }}<button class="btn btn-danger remove"><i class="icon icon-remove"></i> {{$etykieta_usun}}</button>{{END}}
...
<script type="text/javascript">
	...
	$("#add").click(function(){
		...
		$(".prd_dodane_{{$nazwa}} .spn").spinner({min: spinnerMin, step: skok});
	...
	function sprawdzMultipleDodawanie()
	{
		...
		if (m == 'true')
		{
			...
			if(unit == 'gr')
			{
				$(".spn[data-unit=gr]").spinner( { min: {{$min_gr}}, step: {{$skok_gr}}, value: {{$min_gr}} } );
			}
			else
			{
