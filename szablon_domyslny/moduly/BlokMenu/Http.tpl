...
{{BEGIN elementStart}}<li class="{{IF $poziom == '1'}} nav-item dropdown {{ELSE}}{{END}}" {{IF $active}}active{{END}}>{{END elementStart}}
{{BEGIN elementTrescLink}} <a class="{{$klasa}}" href="{{$url}}">{{$nazwa}}</a>{{END}}
