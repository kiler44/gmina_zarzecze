...
 
	...
	$('#s2id_listaTeamowSelect').css('display', 'inline').css('float', 'none');
		...
		$('#zapisz_przydzielenie, #zapisz_przydzielenie_m').live('click', function(){
...
<div class="widget-box container_{{$type}}">
	...
	<div class="widget-content">
		...
		<div class="clear"></div>
		{{$formularz_filtruj}}
		<input id="zapisz_przydzielenie_m" class="btn btn-success btn-large fR" type="submit" value="{{$zapisz_input}}" name="zapisz_przydzielenie">
		<div class="clear"></div>
	...
	<div  id="listaZamowien">
		<div class="widget-title">
			...
			<h5 style="width:60%">{{$lista_zamowien_naglowek}}</h5>
			{{ IF idKoordynatora}}
				{{IF powiazZGETApi}}
			<a href="{{$urlImportIndex}}" class="btn btn-success btn-xs" id="import" style="float: right; margin-top: 4px; margin-right: 5px;" >Import orders from GET</a>
					{{END IF}}
			{{END IF}}
...
<script type="text/javascript">
	...
	function potwierdzPobranoNotatke(dane){
		if(dane.error)
		{
		...
		{
			var wierszKomentarza = '<li><div class="user-thumb"><img src="{ZDJECIE}" alt="" style="cursor: pointer;" width="40" height="40"></div><div class="comments"><span class="user-info"> {DATA} {USER}  </span><p class="red"> {NOTATKA}</p></div></li>';
			var znajdz = [ '{ZDJECIE}', '{DATA}', '{USER}', '{NOTATKA}'];
			var zamien = [dane.zdjecie, dane.data, dane.uzytkownik, dane.komentarz];
			var zamienionyWiersz = wierszKomentarza.replaceArray(znajdz, zamien);
			var komentarzDiv = $('.notatkiBlok').append(zamienionyWiersz);
			$('#pobierzNotatke').hide();
			var iloscNotatek = parseInt($('.iloscNotatek').text());
			iloscNotatek = iloscNotatek + 1;
			$('.iloscNotatek').text(iloscNotatek);
	...
	var iPad = 0;
	$(document).on('click', "#pobierzNotatke", function(e){ ladujPreloader(); var idZamowienia = $(this).attr('id-attr'); ajax("{{$urlPobierzNoteatkeGET}}", potwierdzPobranoNotatke , {idZamowienia: idZamowienia}, 'POST', 'json', false ); });
	$(document).ready(function () {
