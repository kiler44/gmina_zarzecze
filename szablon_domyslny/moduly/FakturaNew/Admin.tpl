...
<script type="text/javascript">
		...
		$('.editable').editable({
			 ...
			 * { 
				...
				callback			: function(data){
					if( data.content ) {
						var id = parseInt(data.$el.effect('blink').attr('id').replace('i-',''));
						var obj = {
							id: id,
							fakturaHeading: data.content
						};
						...
						$.ajax({
							...
							success: function(dane) {
								$('#kontenerKomunikaty').html(dane.komunikaty);
							},
							error: function (xhr, ajaxOptions, thrownError) {
								var error = 'Save invoice heading status: '+xhr.status;
								if (thrownError != '') 
								{
	...
	{
		var sumaBrutto = parseFloat($('#sumaBrutto').text().replace(" ", "").replace(" ", ""));
		var sumaZaplacono = parseFloat($('#sumaZaplacono').text().replace(" ", "").replace(" ", ""));
		...
		$('.'+klasa).each(function()
				{
					...
					{
						suma = parseFloat(suma) + parseFloat($(this).text().replace("{{$kwota_znaczek}}",'').replace(" ", ''));
					}
				}
			);
		...
		$('.'+klasa).each(function()
				{
					...
					{
						suma = parseFloat(suma) + parseFloat($(this).val().replace(" ", ''));
					}
				}
			);
