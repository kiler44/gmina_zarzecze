	...
	<ul id="myTab" class="nav nav-tabs tab-bricky">
		...
		<li class="{{$klasa}}">
			<a href="{{$link}}" data-toggle="">{{IF $ikona}}<i class="icon {{$ikona}}"></i>{{END}} {{$nazwa}} 
				{{IF $wyswietlajIlosc}}<span class="label {{$iloscKlasa}} tip-left" data-original-title="">{{$ilosc}}</span>{{END}}
...
<script type="text/javascript">
	...
	function ustawJakoNaprawiony(idProduktu)
	{
		
		ajax("{{$urlUstawJakoNaprawiony}}" , potwierdzUstawionoNaprawiony, { id: idProduktu }, 'POST', 'json' );
		return false;
	}
	
	function potwierdzUstawionoNaprawiony(dane)
	{
		if(dane.error > 0)
		{ 
	...
	$(document).ready(function () {
		...
		$(document).on('click', '.ustawJakoNaprawiony' , function(e){ ustawJakoNaprawiony($(this).attr('id').replace('doNaprawyId_', ''));	});
...
 
	...
	      	}
		...
		pobierzZawartoscKoszyka();
		
		$('.podgladGrupy').live('click', function(){
			var id = $(this).attr('id').replace('pg_', '');
			
			if($('#podgladGrupyInformacje_'+id).is(':visible'))
				$('#podgladGrupyInformacje_'+id).hide();
			else
				$('#podgladGrupyInformacje_'+id).show();
			
		});
		$(window).scroll(function() {
			
			if($('.koszyk').offset().top < $(window).scrollTop() && !$('.koszyk').hasClass('koszyk_fixed'))
			{
				$('.koszyk').addClass('koszyk_fixed');
			}
			
			var pozycja = 315;
			if(parseInt($(window).scrollTop()) < pozycja && $('.koszyk').hasClass('koszyk_fixed'))
			{
				$('.koszyk').removeClass('koszyk_fixed');
			}
		...
		$('#serwis, #niekompletne').live('click', function(){
			
			$('#nrStrony').val({{$nrStrony}});
			$('#naStronie').val({{$naStronie}});
			
			kategoria = $(this).attr('data-id'); 
			szukaj('', kategoria);
			$('#fraza').val('');
			return false;
		});
		...
		$('.dodajDoKoszyka').live('click', function(){
			var ilosc = $(this).siblings('.wyszukiwarka_spinner').find('.ilosc').val();
			var idProduktu = $(this).attr('id').replace('produktId_', '');
			dodajDoKoszyka(idProduktu, ilosc);
			return false;
		});
		
		$('#podgladKoszykaPrzycisk').live('click', function(){
			
			if($('#podgladKoszyka').is(':visible'))
			{
				$('#podgladKoszyka').hide();
			}
			else
		...
		$('#czyscKoszyk').live('click', function(){
			usunZKoszyka(0);
		});
	...
	function dodajDoKoszyka(idProduktu, ilosc)
	{
		$('#loader').fadeIn(500);
		$.ajax({
				url: "{{$dodajDoZamowieniaLink}}",
				type: 'POST',
				dataType: 'json',
				data: {ilosc:ilosc , idProduktu:idProduktu },
				async: true,
				success: function(dane) {
					$('#loader').fadeOut(500);
					if(dane.succes)
					{
						$('#podgladKoszykaPrzycisk').addClass('koszykWypelniony');
						$('#podgladKoszykaPrzycisk').animate({ opacity: 0.3,  backgroundColor: '#ff0000' , }, 500 )
								  .animate({ opacity: 1,  backgroundColor: '#5cb85c'   } , 200);
	...
	function pobierzZawartoscKoszyka()
	{
		{{IF $wyswietlajKoszyk}}
		$('.koszyk').show();
		{{END}}
		...
		$.ajax({
				url: "{{$pobierzZawartoscKoszykaLink}}",
				type: 'POST',
				dataType: 'json',
				data: {},
				async: true,
				success: function(dane) {
					$('#podgladKoszyka').html(dane.html);
...
<div>
		...
		<div class="fR koszyk" style="display: none;">
			<ul class="quick-actions-horizontal" style="margin:10px auto;">
				<li id="koszyk">
					<a href="#" id="podgladKoszykaPrzycisk" {{IF $koszykIlosc > 0}}class="koszykWypelniony"{{END}} >
					<i class="icon-shopping-cart"></i>
					<span>{{$koszykEtykieta}} (<span id="koszykIlosc">{{$koszykIlosc}}</span>)</span>
					</a>
				</li>
				<div class="popover bottom" id="podgladKoszyka" {{IF koszykRozwiniety}}style="display:block;"{{ELSE}}style="display:none"{{END}}>
					{{BEGIN zawartoscKoszyka}}
					<table cellpadding="5" class="tabelkaKoszyk">
						<thead>
							<tr>
								<th>{{$koszykKodEtykieta}}</th>
								<th>{{$koszykNazwaEtykieta}}</th>
								<th>{{$koszykIloscEtykieta}}</th>
								<th>{{$koszykUsunEtykieta}}</th>
							</tr>
						</thead>
						<tbody>
						{{BEGIN pozycja}}
							<tr>
								<td><i class="icon icon-barcode"></i> {{$produkt_kod}}</td>
								<td>{{$produkt_nazwa}}</td>
								<td>
									<span class="cena qty_full">
										<input type="text" readonly="true" name="produkt[]" max="{{$maxIlosc}}" id="produktIloscId_{{$produkt_id}}" class="produktIlosc qty fL" value="{{$produkt_ilosc}}" />
									</span>
								</td>
								<td>
									<button class="btn btn-danger btn-small usunZKoszyka" id="produktId_{{$produkt_id}}" ><i class="icon icon-minus-sign"></i></button>
								</td>
							</tr>
						{{END}}
						</tbody>
					</table>
					<a class="btn btn-info fR margin" href="{{$koszykFinalizujLink}}">{{$koszykFinalizuj}}</a>
					<button class="btn btn-danger fR margin" id="czyscKoszyk" >{{$czyscKoszyk}}</button>
					{{END}}
					{{BEGIN koszykPusty}}
				...
				<div class="widok-wyszukaj-naglowek">
					<div class="widok-wyszukaj-tytul fL"  >
						<table cellpadding="8px">
							<tr>
								...
								<td>
								{{IF grupa}}
								<button class="btn btn-success podgladGrupy " id="pg_{{$idProduktu}}" title="{{$podgladGrupyEtykieta}}" >
					...
					<div class="widok-wyszukaj-przyciski fR" style="width:30%;" >
						<span class="wyszukiwarka_spinner qty">
							<input type="text" {{IF blokujSpinner}}readonly="true"{{END}} {{IF ustawMaxSpinner}}max="{{$produktIlosc}}"{{END}} class="ilosc" value="1" />
						</span>
						{{IF wyswietlajDodajDoKoszyka}}
						<button id="produktId_{{$idProduktu}}" class="btn btn-info btn-lg dodajDoKoszyka " title="{{$dodajDoZamowieniaEtykieta}}">
						...
						{{IF wyswietlajUstawNaprawiony}}
						<button id="doNaprawyId_{{$idProduktu}}" class="btn btn-warning btn-lg ustawJakoNaprawiony " title="{{$ustawNaprawionyEtykieta}}">
