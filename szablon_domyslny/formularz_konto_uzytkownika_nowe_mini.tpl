...
{{BEGIN input}}
<p class="{{$klasa}}">
	{{BEGIN etykieta}}<label for="{{$nazwa}}" style="width:140px;" class="noBlock">{{$etykieta}}{{if($wymagany,' <strong>*</strong>')}}</label>{{END}}
	{{$html}}
	{{BEGIN blad}}<div class="formularz_blad" style="margin-left: 150px;">{{$tresc}}</div>{{END}}
</p>
{{END}}
