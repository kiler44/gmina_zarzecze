...
<div class="input_container">
	...
	<tr class="{{$klasa}}">
		...
		<td class="pole">
	{{BEGIN pionowy}}<table border="0">
	{{BEGIN pole}}<tr><tr>{{$html}}{{BEGIN blad}}<div class="formularz_blad">{{$tresc}}</div>{{END}}</td></tr>{{END}}
	</table>{{END}}
	{{BEGIN poziomy}}<table border="0"><tr>
	{{BEGIN pole}}<td>{{$html}}{{BEGIN blad}}<div class="formularz_blad">{{$tresc}}</div>{{END}}</td>{{END}}
	</tr></table>{{END}}
		</td>
	</tr>
	</tbody>
